...
OUTDIR=/home/taritree/larbys/data/mcc8.1/nue_1eNpfiltered/out_week0626/tagger/
...
do
    ...
    echo "launching job=$i" && singularity exec $SINGULARITY_IMG bash -c "export SLURM_PROCID=$i && cd ${WORKDIR} && source run_job.sh ${TAGGER_CFG} ${INPUTLISTS} ${OUTDIR} ${JOBIDLIST}" >> log_mccaffery_job.txt 2>&1 &
done
