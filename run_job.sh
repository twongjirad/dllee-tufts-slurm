...
slurm_folder=`printf slurm_tagger_job%03d ${jobid}`
mkdir -p ${slurm_folder}
cd ${slurm_folder}/

# copy over input list
inputlist_larcv=`printf ${inputlist_dir}/input_larcv_%03d.txt ${jobid}`
inputlist_larlite=`printf ${inputlist_dir}/input_larlite_%03d.txt ${jobid}`
